...
# === SYM DISCOVERY FALLBACK ===
try:
    DATA_DIR
except NameError:
    DATA_DIR = "/workspace/data/processed"

# إن لم يكن sym معرفًا بواسطة كتل أخرى، نكتشفه من processed/
if 'sym' not in globals():
    import os, glob, streamlit as st
    files = sorted(glob.glob(os.path.join(DATA_DIR, "*_latest_signal.json")))
    discovered = [os.path.basename(p).split("_")[0] for p in files]
    options = discovered or ["2010.SR","1120.SR","7010.SR"]
    sym = st.sidebar.selectbox("الرمز (اكتشاف تلقائي)", options, index=0)
...
st.markdown("---")
st.subheader("📈 Backtest Summary (cumulative)")
_bts = os.path.join(DATA_DIR, "backtest_summary.csv")
if os.path.exists(_bts):
    import pandas as _pd
    _df = _pd.read_csv(_bts)
    st.dataframe(_df, use_container_width=True, height=260)
else:
    st.caption("لم يتم توليد ملخّص الباك تست التراكمي بعد.")
...
st.markdown("---")
st.subheader("📈 Backtest Summary — تراكمي (الصفحة الرئيسية)")

PROC = "/workspace/data/processed"
bs = os.path.join(PROC, "backtest_summary.csv")
if os.path.exists(bs):
    df = _pd.read_csv(bs)
    # طبع الأعمدة إلى lower-case ثم نضمن وجود 'symbol'
    lower_map = {c.lower(): c for c in df.columns}
    symcol = lower_map.get('symbol') or lower_map.get('ticker')
    if symcol and symcol != 'symbol':
        df = df.rename(columns={symcol: 'symbol'})
    elif 'symbol' not in df.columns:
        # محاولة أخيرة: لو فيه عمود اسمه 'Symbol'
        if 'Symbol' in df.columns:
            df = df.rename(columns={'Symbol':'symbol'})
    # إن لم نجد عمود الرمز نوقف العرض بأمان
    if 'symbol' not in df.columns:
        st.warning('لم أتعرف على عمود الرمز في backtest_summary.csv')
        st.stop()
    # محاولة استخراج الدول من ملفات profile إن وجدت
    countries = {}
    ...
    if countries:
        ...
        cset = ["الكل"] + sorted(set(df["country"]) - {"—"}) + (["—"] if "—" in set(df["country"]) else [])
        pick = st.selectbox("فلترة حسب الدولة/السوق", cset, index=0)
        if pick != "الكل":
            df = df[df["country"]==pick]
    st.dataframe(df, use_container_width=True, height=300)
else:
    st.info("لا يوجد backtest_summary.csv بعد. شغّل التحديث أو انتظر الخدمة الدورية.")
